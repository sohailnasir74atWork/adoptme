...
//   3. supabase/003_chat_metadata.sql already applied.
//   4. `npm install firebase-admin @supabase/supabase-js --no-save` (or
//      let the existing functions/ install satisfy them — this script
//      just resolves them from there if not present).
...
// Default RTDB URL — adjust if you point at a non-default instance.
...
async function upsertInBatches(table, rows, conflictKey) {
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    if (error) {
      console.error(`❌ ${table} batch ${i / BATCH_SIZE} failed:`, error.message);
      throw error;
    }
...
// Page size for the parent-level scan. Reads N users' subtrees at a time
// instead of pulling the whole tree in one .once('value') (which hangs on
// large datasets — the SDK buffers the entire response before resolving).
const PAGE_USERS = 100;

// Partition support — set RANGE_START / RANGE_END env vars to scope a
// worker to a key range. Used for parallelising the backfill across N
// processes that share the keyspace by lexicographic UID prefix.
...
async function paginateAndUpsert({ rootPath, table, conflictKey, makeRow, childKeyName }) {
  ...
  let totalUpserted = 0;
  let pendingRows = [];
  const FLUSH_AT = 150;  // smaller payload → fewer transient fetch failures
  ...
  while (true) {
    ...
    // startAt is inclusive — drop the lastKey we already processed
    ...
    // Flush in 500-row chunks so memory stays bounded even on huge users.
...
async function upsertSlice(table, slice, conflictKey) {
  // Retry with exponential backoff. Transient `fetch failed` errors
  // happen on long-running backfills — connection drops, intermittent
  // CDN hiccups, occasional rate-limit blips. Upserts are idempotent
  // (on conflict do update), so retrying is safe.
  // 10 retries, ~4 min total backoff window — survives a sustained Supabase
  // edge blip without killing the worker.
...
async function backfillGroupMeta() {
  return paginateAndUpsert({
...
(async () => {
  try {
    const a = await backfillChatMeta();
    const b = await backfillGroupMeta();
