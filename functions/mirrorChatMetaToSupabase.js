 ...
 * mute toggles, lastMessage updates straight to RTDB so notifyNewMessage
 * and old app versions are unaffected. This function fans those writes
 * out to Supabase so new clients can subscribe there for the chat list /
 * unread badge stream and we can drop a chunk of RTDB egress.
 *
...
exports.mirrorChatMetaToSupabase = functions
  ...
  .onWrite(async (change, context) => {
    ...
    if (!change.after.exists()) {
      ...
      if (error) {
        console.error('[mirrorChatMeta] delete failed:', error.message);
      }
      return null;
    }
