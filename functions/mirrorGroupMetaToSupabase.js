 ...
 * into Supabase.
 *
 * Sister of mirrorChatMetaToSupabase — same pattern, different node.
 * RTDB stays source of truth so notifyGroupMessage,
 * /activeGroupChats presence, and old app versions all keep working.
 *
 * Trigger: onWrite at the row level so every field change (mute,
 * lastMessage, unreadCount, member_count, etc.) lands in one upsert.
 *
 * Deployment:
...
exports.mirrorGroupMetaToSupabase = functions
  ...
  .onWrite(async (change, context) => {
    ...
    // Row removed — user left the group, group cleanup ran, etc.
    if (!change.after.exists()) {
      const { error } = await supabase
        ...
        .eq('group_id', groupId);
      if (error) {
        console.error('[mirrorGroupMeta] delete failed:', error.message);
      }
      return null;
    }
    ...
    if (error) {
      console.error('[mirrorGroupMeta] upsert failed:', error.message, {
        userId,
        groupId,
      });
    }
