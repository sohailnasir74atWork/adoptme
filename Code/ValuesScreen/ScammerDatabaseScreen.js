...
const statusLabel = (s, t) => {
    switch (s) {
        case 'verified': return t('scammer.status_verified');
        default: return t('scammer.status_unverified');
    }
};
...
const ScammerDatabaseScreen = () => {
    ...
    const submitReport = useCallback(async () => {
        ...
        const doSubmit = async () => {
            try {
                ...
                if (!parsed.isValid()) {
                    Alert.alert(t('scammer.error_title'), t('scammer.error_invalid_date'));
                    setSubmitting(false);
                    return;
                }
                ...
                if (reportImages.length > 0) {
                    ...
                    try {
                        ...
                        Alert.alert(t('scammer.error_title'), t('scammer.error_image_upload'));
                        setSubmitting(false);
                ...
                Alert.alert(t('scammer.submitted_title'), t('scammer.submitted_msg'));
                // Reset form
                setReportUsername('');
                ...
                if (err?.message === 'RATE_LIMIT') {
                    Alert.alert(t('scammer.rate_limit_title'), t('scammer.rate_limit_msg'));
                } else {
                    console.error('[Scammer] Submit error:', err);
                    Alert.alert(t('scammer.error_title'), t('scammer.error_submit'));
                }
            } finally {
                setSubmitting(false);
                setUploadingImages(false);
    ...
    const approveReport = useCallback(async (reportId) => {
        ...
        try {
            ...
            Alert.alert(t('scammer.done'), t('scammer.verified_msg'));
        } catch (err) {
            console.error('[Scammer] Approve error:', err);
            Alert.alert(t('scammer.error_title'), t('scammer.error_approve'));
        } finally {
            setModAction(null);
        }
            ...
            { text: t('scammer.cancel'), style: 'cancel' },
            {
                text: 'Delete', style: 'destructive', onPress: async () => {
                    ...
                    try {
                        ...
                        setDetailData(null);
                        Alert.alert(t('scammer.done'), t('scammer.deleted_msg'));
                    } catch (err) {
                        console.error('[Scammer] Delete error:', err);
                        Alert.alert(t('scammer.error_title'), t('scammer.error_delete'));
                    } finally {
                        setModAction(null);
                    }
                },
        ...
        >
            <View style={s.listRow}>
                ...
                <View style={[s.badge, { backgroundColor: statusColor(item.status) + '20' }]}>
                    ...
                    <Text style={[s.badgeText, { color: statusColor(item.status) }]}>{statusLabel(item.status, t)}</Text>
                </View>
            </View>
        </TouchableOpacity>
    ), [C, openDetail]);
    ...
    const ListFooter = useMemo(() => {
        ...
        if (summaries.length === 0 && !loading) {
            return <Text style={[s.emptyText, { color: C.muted }]}>{t('scammer.no_reports')}</Text>;
        }
        return null;
        ...
        <View style={[s.container, { backgroundColor: C.bg }]}>
            ...
            <View style={[s.searchRow, { backgroundColor: C.input, borderColor: C.border }]}>
                ...
                <TextInput
                    ...
                    placeholder={t('scammer.search_placeholder')}
            ...
            <View style={s.filterRow}>
                {FILTERS.map((f) => (
                    <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                    >
                        <Text style={[s.filterTabText, activeFilter === f && s.filterTabTextActive]}>{t(`scammer.filter_${f.toLowerCase()}`)}</Text>
                    </TouchableOpacity>
                ))}
            </View>
                    ...
                    <View style={[s.modalContent, { backgroundColor: C.card }]}>
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <View style={s.modalHeader}>
                                <Text style={[s.modalTitle, { color: C.text }]}>{t('scammer.report_details')}</Text>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={[s.detailSub, { color: C.sub }]}>
                                                {t('scammer.scam_date_label')} {selectedReport.scamDate ? fmtDate(selectedReport.scamDate) : t('scammer.na')}
                                            </Text>
                                        </View>
                                        <View style={[s.badge, { backgroundColor: statusColor(selectedReport.status) + '20' }]}>
                                            ...
                                            <Text style={[s.badgeText, { color: statusColor(selectedReport.status) }]}>{statusLabel(selectedReport.status, t)}</Text>
                                        </View>
                                    </View>
                                </View>
                            )}
                            ...
                            {loadingDetail ? (
                                ...
                                <View>
                                    ...
                                    <View style={[s.detailField, { borderColor: C.border }]}>
                                        <Text style={[s.detailLabel, { color: C.sub }]}>{t('scammer.what_happened')}</Text>
                                        <Text style={[s.detailValue, { color: C.text }]}>{detailData.whatHappened || t('scammer.na')}</Text>
                                    </View>

                                    {/* Proof images */}
                                    {detailData.proofUrls?.length > 0 && (
                                        <View style={{ marginTop: 8 }}>
                                            <Text style={[s.detailLabel, { color: C.sub, marginBottom: 6 }]}>{t('scammer.proof_screenshots')}</Text>
                                    ...
                                    {isMod && selectedReport && (
                                        <View style={s.modActions}>
                                            <Text style={[s.detailLabel, { color: C.sub, marginBottom: 8 }]}>{t('scammer.mod_actions')}</Text>
                                                ...
                                                >
                                                    {modAction === 'approve' ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <Text style={s.modBtnText}>{t('scammer.verify_btn')}</Text>
                                                        </>
                                                    )}
                                                </TouchableOpacity>
                                                <TouchableOpacity
                                                ...
                                                >
                                                    {modAction === 'reject' ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <Text style={s.modBtnText}>{t('scammer.delete_btn')}</Text>
                                                        </>
                                                    )}
                                                </TouchableOpacity>
                                            </View>
                                ...
                                <Text style={[s.emptyText, { color: C.muted }]}>{t('scammer.no_details')}</Text>
                            )}
                        </ScrollView>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.modalOverlay}>
                    <View style={[s.modalContent, { backgroundColor: C.card, maxHeight: '90%' }]}>
                        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                            {/* Header */}
                            <View style={s.modalHeader}>
                                <Text style={[s.modalTitle, { color: C.text }]}>{t('scammer.report_scammer')}</Text>
                            ...
                            <View style={[s.banner, { backgroundColor: C.bannerBg, marginBottom: 12 }]}>
                                <Text style={[s.bannerText, { color: C.bannerText, fontSize: 11 }]}>
                                    {t('scammer.safety_note')}
                                </Text>
                            </View>

                            {/* Username */}
                            <Text style={[s.formLabel, { color: C.sub }]}>{t('scammer.username_label')}</Text>
                            <TextInput
                                value={reportUsername}
                                onChangeText={setReportUsername}
                                placeholder={t('scammer.username_placeholder')}
                                placeholderTextColor={C.muted}
                            ...
                            {/* Scam Date */}
                            <Text style={[s.formLabel, { color: C.sub }]}>{t('scammer.scam_date')}</Text>
                            <TextInput
                                value={reportScamDate}
                                onChangeText={setReportScamDate}
                                placeholder={t('scammer.scam_date_placeholder')}
                                placeholderTextColor={C.muted}
                                style={[s.formInput, { backgroundColor: C.input, color: C.text, borderColor: C.border }]}
                            ...
                            {/* Scam Type */}
                            <Text style={[s.formLabel, { color: C.sub }]}>{t('scammer.scam_type_label')}</Text>
                            ...
                            {/* What happened */}
                            <Text style={[s.formLabel, { color: C.sub }]}>{t('scammer.what_happened_label')} <Text style={{ fontWeight: '400', fontSize: 10 }}>{t('scammer.max_chars')}</Text></Text>
                            <TextInput
                                value={reportWhat}
                                onChangeText={(val) => setReportWhat(val.slice(0, 200))}
                                placeholder={t('scammer.what_placeholder')}
                                placeholderTextColor={C.muted}
                            ...
                            {/* Proof images */}
                            <Text style={[s.formLabel, { color: C.sub }]}>{t('scammer.proof_label')} <Text style={{ fontWeight: '400', fontSize: 10 }}>{t('scammer.proof_optional')}</Text></Text>
                            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10, flexWrap: 'wrap', gap: 8 }}>
                                ...
                                {reportImages.length < 3 && (
                                    <TouchableOpacity onPress={handlePickProofImage} style={[s.addProofBtn, { borderColor: C.border }]}>
                                        ...
                                        <Text style={{ color: C.sub, fontSize: 10, marginTop: 2 }}>{t('scammer.add_image')}</Text>
                                    </TouchableOpacity>
                                )}
                            </View>
                            ...
                            >
                                {submitting ? (
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        <Text style={s.submitBtnText}>{uploadingImages ? t('scammer.uploading_images') : t('scammer.submitting')}</Text>
                                    </View>
                                ) : (
                                    <Text style={s.submitBtnText}>{t('scammer.submit_report')}</Text>
                                )}
                            </TouchableOpacity>
                        </ScrollView>
                    </View>
