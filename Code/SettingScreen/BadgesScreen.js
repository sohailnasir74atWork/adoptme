 ...
 * Shows: XP progress, all 22 badges, level roadmap, streak status
 * 📅 2026-03-13: Created for dedicated badge/achievement viewing
 * 📅 2026-03-13: Added shareable badge card (ViewShot + react-native-share)
 * 📅 2026-03-14: Premium UI cleanup — cleaner cards, tighter spacing
 * 📅 2026-03-15: Converted from Modal to navigation screen
 */
  ...
  return (
    <View style={[s.xpCard, { backgroundColor: isDark ? '#1e293b' : '#fff' }]}>
      <View style={s.xpTopRow}>
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={[s.xpSubtitle, { color: isDark ? '#64748b' : '#94a3b8' }]}>
            {t('badges_screen.xp.level_and_xp', { level: currentLevel.level, xp: xp.toLocaleString() })}
          </Text>
        </View>
        {!isMax && (
          <View style={s.xpNextBadge}>
            <Text style={{ fontSize: 9, color: isDark ? '#475569' : '#94a3b8', fontWeight: '600' }}>{t('badges_screen.xp.next')}</Text>
      ...
      <Text style={[s.progressLabel, { color: isDark ? '#475569' : '#94a3b8' }]}>
        {isMax ? t('badges_screen.xp.max_level') : t('badges_screen.xp.to_next_level', { xp: (nextLevel.xp - xp).toLocaleString() })}
      </Text>
    </View>
  );
};
...
const BadgeCard = ({ badgeId, earned, isDark }) => {
  ...
  // Stronger backgrounds for earned badges
    ...
    <View style={[
      ...
      {earned ? (
        <View style={[s.earnedTag, { backgroundColor: badge.color + '25' }]}>
          <Text style={[s.earnedTagText, { color: badge.color }]}>{t('badges_screen.badge.earned')}</Text>
        </View>
      ) : (
    ...
    <View style={[
      ...
      <View style={{ flex: 1 }}>
        <Text style={[s.levelName, {
          ...
          {t('badges_screen.level.title_format', { level: level.level, title: level.title })}
        </Text>
        <Text style={[s.levelXp, { color: isDark ? '#475569' : '#94a3b8' }]}>
          {t('badges_screen.level.xp', { xp: level.xp.toLocaleString() })}
        </Text>
      </View>
      {isCurrent && (
        <View style={s.currentTag}>
          <Text style={s.currentTagText}>{t('badges_screen.level.you')}</Text>
        </View>
      )}
    </View>
  );
    ...
    <View style={[s.section, { backgroundColor: isDark ? '#1e293b' : '#fff' }]}>
      ...
      >
        <Text style={s.claimButtonText}>
          {canClaim ? t('badges_screen.streak.claim') : t('badges_screen.streak.view')}
        </Text>
      </TouchableOpacity>
    </View>
  );
...
const BadgesScreen = ({ navigation }) => {
  ...
  // Badge tiers config
  const tiers = [
    { tier: 3, label: t('badges_screen.tiers.elite_label'), desc: t('badges_screen.tiers.elite_desc') },
    { tier: 2, label: t('badges_screen.tiers.pro_label'), desc: t('badges_screen.tiers.pro_desc') },
    { tier: 1, label: t('badges_screen.tiers.starter_label'), desc: t('badges_screen.tiers.starter_desc') },
  ];
      ...
      <View style={[s.container, { backgroundColor: isDark ? '#0f172a' : '#f1f5f9' }]}>
        ...
        >
          ...
          >
            ...
            <Text style={s.shareBtnText}>{t('badges_screen.share.btn_text')}</Text>
          </TouchableOpacity>
          ...
          <ViewShot ref={shareRef} options={{ format: 'png', quality: 0.9 }}>
            <View style={s.shareCard}>
                  ...
                  <>
                    ...
                    {earnedCount > 6 && (
                      <View style={[s.shareCardBadge, { backgroundColor: 'rgba(99,102,241,0.15)', borderColor: 'rgba(99,102,241,0.3)' }]}>
                        ...
                        <Text style={[s.shareCardBadgeName, { color: '#818cf8' }]}>{t('badges_screen.share_card.more')}</Text>
                      </View>
                    )}
                  </>
                ) : (
                  <Text style={s.shareCardEmpty}>{t('badges_screen.share_card.empty')}</Text>
                )}
              </View>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                ...
                <Text style={s.shareCardFooter}>adoptmevalues.app</Text>
                <Text style={{ fontSize: 14 }}>🐣</Text>
              </View>
            </View>
          </ViewShot>
