 ...
 * for the first available game: Quiz → Spin → Memory.
 */
...
// ── Game config ──
const GAMES = [
  { key: 'quiz',   emoji: '🧠', label: 'Daily Quiz ready!',     color: '#8B5CF6', field: 'lastQuizAt' },
  { key: 'spin',   emoji: '🎡', label: 'Free spin available!',   color: '#F59E0B', field: 'lastSpinAt' },
  { key: 'memory', emoji: '🃏', label: 'Memory Match — play now!', color: '#10B981', field: 'lastMemoryAt', limitField: 'memoryPlaysToday', limit: 1 },
];
  ...
  const checkGames = useCallback(async () => {
    ...
    try {
      ...
      for (const game of GAMES) {
        ...
        if (game.limitField) {
          // Memory: check plays today
          const plays = data[game.limitField] || 0;
          if (!isSameDay(lastPlay) || plays < game.limit) {
            setBannerData(game);
